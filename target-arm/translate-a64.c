...
#define TRACE_DECODE(fmt, ...)                                \
    do {                                                                \
        fprintf(stderr, "%s: 0x%08x @ %" HWADDR_PRIx " " fmt "\n",      \
                __func__, insn, s->pc-4, ## __VA_ARGS__);              \
    } while (0);
#else
#define TRACE_DECODE(fmt, ...)    do { /* nothing */ } while (0);
#endif
                                    ...
                                    int size, int rn, int rd)
{
    ...
    if (is_double) {
        ...
        for (pass = 0; pass < (is_scalar ? 1 : 2); pass++) {
            ...
            switch (opcode) {
            case 0x3d: /* FRECPE */
                gen_helper_frecpe_f64(tcg_res, tcg_op, fpst);
                break;
            case 0x3f: /* FRECPX */
        ...
        for (pass = 0; pass < maxpasses; pass++) {
            ...
            case 0x3c: /* URECPE */
                ...
                break;
            case 0x3d: /* FRECPE */
                gen_helper_frecpe_f32(tcg_res, tcg_op, fpst);
                break;
            case 0x3f: /* FRECPX */
...
static void handle_simd_shift_intfp_conv(DisasContext *s, bool is_q, bool is_u,
                                         int immh, int immb, int opcode,
                                         int rn, int rd)
{
...
static void disas_simd_shift_imm(DisasContext *s, uint32_t insn)
{
    ...
    case 0x1c: /* SCVTF / UCVTF */
        handle_simd_shift_intfp_conv(s, is_q, is_u, immh, immb, opcode, rn, rd);
        break;
